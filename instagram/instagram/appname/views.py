...
def signin(request):
    if request.method == "POST":
        ...
        form = SigninForm()
        return render(request, 'appname/signin.html',{'sign_in_form':form})
...
def mypage(request, user_id):
    m_user = CustomUser.objects.get(username=user_id)
    posts =Post.objects.filter(writer=m_user)
    return render(request, 'appname/mypage.html', {'posts': posts, 'm_user': m_user})